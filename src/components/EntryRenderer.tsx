...
import { Entry, EntryType, EntryStatus, EntryVisibility } from '../types';
import { isArabicText, parseInlineFormatting, ContentBlock, parseContentToBlocks } from '../utils';
...
export function EntryRenderer({
  ...
  // Global shortcut handler for Bold (Ctrl+B) and Italic (Ctrl+I)
  useEffect(() => {
    if (mode !== 'edit') return;
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Update native value and dispatch event to keep React state synchronized
      ...
      // Dispatch 'input' event so react state captures the update
  ...
  const handleTitleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (mode === 'edit') {
      ...
      triggerSave(content, footnotes, marginNotes, contentType, status, visibility, tags, generatedSlug, val);
    }
  };
            ...
            {(contentType === 'Note' || contentType === 'Essay') && (
              <div className="space-y-6">
                {paragraphs.map((p, idx) => {
                  const block = parseContentToBlocks(p)[0] || { type: 'paragraph', text: p };
                  ...
                  <div className="flex justify-end gap-2 pt-1">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (text.trim()) {
                          insertQuoteAtCursor(`<quote type="latin"><text>${text.trim()}</text></quote>`);
                        }
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (arabic.trim()) {
                          ...
                          insertQuoteAtCursor(`<quote type="arabic">\n  <arabic>${arabic.trim()}</arabic>${transXml}\n</quote>`);
                        }
                        setActiveQuoteInsert(null);
                      }}
                      className="px-3 py-1 font-mono text-[9px] uppercase bg-adjung-maroon text-white rounded hover:bg-adjung-maroon/90 font-medium cursor-pointer shadow-sm"
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#1e1c18] text-stone-100 border-t border-stone-800 shadow-2xl py-3.5 px-4 md:px-8 backdrop-blur-md bg-opacity-95">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 bg-stone-800/80 px-2 py-1 rounded border border-stone-700/60">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                onClick={() => {
                  setStatus('Draft');
                  triggerSave(content, footnotes, marginNotes, contentType, 'Draft');
              ...
              <button
                ...
                onClick={() => {
                  ...
                  // update publishedDate if it was not set
                  const pubDate = entry.publishedDate || new Date().toISOString();
                  triggerSave(content, footnotes, marginNotes, contentType, 'Published', 'Public', tags, slug, title);
              ...
              <button
                type="button"
                onClick={() => {
                  triggerSave(content, footnotes, marginNotes, contentType, status, visibility, tags, slug, title);
              ...
              <button
                ...
                onClick={() => {
                  setStatus('Archived');
                  triggerSave(content, footnotes, marginNotes, contentType, 'Archived');
                  showToast('Changes saved', 'success');
                }}
                className={`px-3.5 py-1.5 text-[10px] font-mono uppercase tracking-wider rounded transition-all font-medium border border-transparent cursor-pointer ${
                  status === 'Archived'
