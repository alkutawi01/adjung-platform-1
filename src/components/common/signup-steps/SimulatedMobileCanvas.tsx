...
export default function SimulatedMobileCanvas({ onSave, onCancel }: SimulatedMobileCanvasProps) {
  ...
  return (
    <div className="bg-[#1A1816] text-stone-100 p-5 rounded-2xl max-w-[280px] w-full border border-stone-800 shadow-2xl space-y-4 font-sans select-none relative">
      ...
      <div className="flex justify-between items-center border-b border-stone-800/90 pt-2 pb-2">
      ...
      <div className="space-y-1">
        <p className="text-[11px] text-stone-300 font-sans text-center font-normal">Please sign inside the following box:</p>
      </div>
      ...
      <div className="bg-stone-950 border border-stone-800 rounded-lg h-44 relative overflow-hidden">
        ...
        />
        {strokes.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-30 text-center px-4">
            <span className="font-sans text-[12px] text-stone-300 font-normal">Use your finger or stylus</span>
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button
          ...
          className="flex-1 py-2 bg-adjung-maroon hover:bg-[#962c3e] disabled:opacity-50 disabled:cursor-not-allowed text-stone-100 font-mono text-[10px] uppercase rounded-sm transition-all cursor-pointer font-bold"
        >
          Send to PC
        </button>
      </div>
