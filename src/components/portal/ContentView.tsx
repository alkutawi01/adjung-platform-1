...
import { Rss, SlidersHorizontal, X, Search } from 'lucide-react';
...
export function ContentView({ entries, users, setSelectedEntry, setSelectedAuthorId }: ContentViewProps) {
  ...
  // A search-first filter picker, not a browse-everything list — the
  // sidebar version of this (an auto-listed page of every writer/topic)
  // doesn't survive past a handful of either. At any scale, this only ever
  // renders a small "popular" starter set or a capped set of live matches,
  ...
  return (
    // Single centered column, same width whether or not filters are open —
    // a persistent sidebar column was pushing the feed off-center instead
    // of leaving it with the same margins on both sides. The filter picker
    // below is an overlay, not a layout column.
    <div className="max-w-2xl mx-auto space-y-8 text-left relative">
      <div className="border-b border-[#111111]/10 pb-5 flex items-start justify-between gap-4">
        ...
        <button
          ...
          className={`shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded border text-[11px] font-mono uppercase tracking-wider transition-colors ${
      ...
      {/* Filter picker — an overlay dropdown anchored under the button, not
          a layout column. Closing it (backdrop click) keeps whatever was
          already selected; it's just where you go to change the selection. */}
      {showFilterPanel && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setShowFilterPanel(false)} aria-hidden="true" />
          <div className="absolute right-0 top-16 z-50 w-full sm:w-80 bg-white border border-stone-200 rounded-lg shadow-lg p-4 space-y-4">
