...
### 12. Kotak Tetapan Waktu Refresh & Caching Pintar Google Doc
- **Kotak Input Waktu Tersuai (Custom Sync Times Box)**: Menambah medan input baharu *"Jadual Refresh Google Doc"* di dalam [Editorium.tsx](file:///c:/Users/alkut/adjung-platform-1/src/components/studio/Editorium.tsx) di bawah bahagian curation. Pilihan ini membenarkan anda menaip jadual tersuai mengikut format 24j yang dipisahkan dengan koma (contoh: `12:10, 00:10`).
- **Logik Smart Caching**: Kandungan Google Doc yang dimuat turun kini disimpan terus sebagai cache di dalam Firestore di bawah medan baharu `inTheNewsCachedText` dll. Pelawat tidak lagi membuat panggilan API terus ke Google Doc semasa memuatkan halaman (menjimatkan bandwidth luar secara drastik dan halaman dibuka serta-merta).
- **Auto-Fetch Jam Tersuai**: Di dalam [AppContext.tsx](file:///c:/Users/alkut/adjung-platform-1/src/context/AppContext.tsx), fungsi `refreshDbState` akan membandingkan waktu sekarang dan waktu muat turun terakhir dengan jadual tersuai anda menggunakan pembahagi waktu dinamik `getMostRecentSyncThreshold` di [utils.tsx](file:///c:/Users/alkut/adjung-platform-1/src/utils.tsx). Sekiranya waktu refresh terdekat sudah berlalu, barulah sistem melakukan auto-fetch secara senyap di latar belakang.
- **Kemas Kini Manual Terjamin**: Butang merah **Update In The News Digest** (dan butang curation lain) akan sentiasa menetapkan `LastFetched` kepada kosong, memaksa (force) muat semula data dari Google Doc serta-merta tanpa mengira jadual.
