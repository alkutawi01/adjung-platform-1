...
// 1. Inject state
const stateTarget = `const [expandedFrontpageNotes, setExpandedFrontpageNotes] = useState<string[]>([]);`;
...
if (content.includes(stateTarget) && !content.includes('frontpageCarouselIndex')) {
  content = content.replace(stateTarget, stateTarget + stateInjection);
}

// 2. Find start of Frontpage Module
const frontpageComment = `{/* ACTIVE MODULE 0B: CURATED FRONTPAGE (Platform public index of publications & scholars) */}`;
const startIndex = content.indexOf(frontpageComment);
if (startIndex === -1) {
  console.error("Could not find frontpage comment start!");
  process.exit(1);
}

// 3. Find </main> after the frontpage comment
const endComment = `</main>`;
          ...
          return (
            <div className="max-w-4xl mx-auto select-none animate-fade-in space-y-24 py-16 px-4">
              ...
              <div className="text-center pt-8">
                <h1 className="font-serif text-5xl md:text-6xl font-light text-stone-900 tracking-tight mb-4">Adjung</h1>
                <span className="font-mono text-[10px] text-stone-500 uppercase tracking-[0.3em]">Rebuilding the civilization</span>
              </div>
...
fs.writeFileSync(appPath, prefix + newFrontpageBlock + suffix, 'utf8');
console.log("Successfully replaced Frontpage module safely.");
