...
| Index / directory of all published works | ✅ Built — `EditorialIndex.tsx` already read live Supabase-backed `entries`, not mockDb (this note was stale); §14.1 closed the real gaps: body-content search, finite pagination, shareable URL, `getIndexEntries()` abstraction. `Directory.tsx` unchanged. | MVP |
...
### 14.1 Do now — required for a real, personal-use MVP — ✅ DONE (2026-08-29, see §15)
...
## 15. Session Log — 2026-08-28/29: §14.1 closed, Content shipped, Note gap closed

**§14.1 is now fully done** — all three do-now items (authoritative metadata, `getIndexEntries()` abstraction, finite Index UX) shipped and verified live in this session. Fixes below were found via live testing (browser automation against the deployed app, not just code reading) and a real classical-text (Jawi/hadith) content simulation.

### Bugs found and fixed
- **`assign_entry_serial_no()` trigger broke ALL publishing** — combined `FOR UPDATE` with `max()` in one query (Postgres rejects this); every Publish silently stayed a draft after the §14.1 migration landed. Split into a lock statement + a separate read. Regression from this session's own earlier work, caught immediately via live testing.
- **Margin note italic by default** — `ElasticMarginRow.tsx`'s reading-view text was hardcoded `italic`. Removed; violates the no-decorative-italics rule.
- **Desk had no way to create a Note** — `WritingDesk.tsx` only ever rendered `+ Essay`. `createNewEntry('Note')` was already correct (no title, no margin notes) but unreachable from the UI. Added `+ Note`. Also fixed the draft/published card title going blank for a title-less Note (now falls back to a content preview).
- **Note rendered as a shrunk Essay everywhere** — drop cap, centered/justified layout, serif "voice" font. Research (Smashing Magazine on drop-cap convention; a Nieman Lab reader survey) backs treating Note as short-form (X/Threads register): no drop cap, left-aligned, handwritten voice font (`font-handwritten`), warm-paper card tint + a `NOTE` pill instead of an empty title slot. Applied consistently to Folio, Content, and Frontpage's Featured Notes card. Index's plain table rows were deliberately left alone — it's a utilitarian catalog, not a reading surface.
- **Frontpage's four curation slots (splash, Editor's Selections, Featured Essays, Featured Notes) showed a fake demo entry / "EMPTY SLOT" placeholders** even with 15+ real published entries, because they only ever read `systemSettings.*Ids` with no fallback — unlike the same file's own "Newest Essays" section, which already did this correctly. All four now backfill from a shared newest-first `publishedPool` (via a `usedEntryIds` set, so nothing gets duplicated across slots) when curation hasn't happened yet, exactly matching the pattern Newest Essays already used.
- **Raw XML/markdown leaking into every preview surface that reads `content` directly** — an entry opening with a heading (`## ...`) or an XML `<quote type="arabic">...</quote>` block (used for hadith/verse citations before the author's own prose — a real structure, not a hypothetical) got its markup cut mid-tag by naive `content.substring()`/word-truncation, leaking literal `<arabic>...`/`##...` text. Fixed in three places that each had their own version of this bug: `TimelineEntryCollapseRenderer.tsx`'s Folio card truncation, and Frontpage's splash/Editor's-Selections/Featured-Notes excerpt fallbacks. Extracted the cleanup into one shared `flattenBlocksForPreview()` in `utils.tsx` so future preview surfaces reuse it instead of re-inventing (badly).
- **arabic-quote's original text was italic with nobody having asked for it** — a bare, unscoped `blockquote { font-style: italic; ... }` in `index.css` (meant only for the editor's own markdown-blockquote preview, already correctly scoped under `#editorial-canvas-editor` via a duplicate selector) bled onto every `<blockquote>` in the app. Scoped the rule properly; added an explicit `not-italic` on the Arabic paragraph as a safeguard. Also found and fixed: the translation under an arabic-quote was explicitly italic (no-decorative-italics violation) with extra `mt-4/pt-4/border-t` spacing latin-quote's translation doesn't have — removed the italic, tightened to match latin-quote's `mt-2`.
- **Index's "Title" column literally said "Title" on every row**, including Note rows where there is no title (that cell shows a content excerpt) — renamed to "Title / Excerpt". Also capped an uncapped-length Note preview (an unusually long first sentence with no punctuation could blow out the row).
- **Desk's draft/published card titles used a different font (`font-sans`) than the essay title everywhere else** (composer, Folio, Frontpage all use `font-serif`/Source Serif 4) — same title, two different typefaces depending on which screen showed it. Unified to `font-serif`.
- **Content nav had no reliable way in** — added to `Navbar.tsx`'s top bar (gated the same as Directory/Index, on `selectedAuthorId === ''`) but the *actual* account-dropdown menu (built directly in `Navbar.tsx`, separate from the dead `getNavigationItems()` in `App.tsx`) only had Account/Editorium/Switch Scriptor/Sign Out — added Content/Directory/Index there too, since `selectedAuthorId` is pinned to the current user's own site by default on this platform.

### New: Content (shipped this session, not previously in this matrix)
A chronological Note+Essay feed, explicitly paired with Frontpage as the "portal" surface (Directory+Index is the other pair — a catalogue, never a feed). Locked scope: mixed Note+Essay, newest-first, manual Load More (no infinite auto-scroll — same finite-pagination principle as Index), no filters. Backed by `getContentEntries()` in `utils/getContentEntries.ts`, same seam pattern as `getIndexEntries()`. Nav-gated the same as Frontpage (no permission required, any signed-in user).

### Resolved since first logged
- ~~Desk's draft/published list layout~~ — redesigned to a dense row list (type | title | date | icon) per Izzat-approved mockup; also fixed the list never being explicitly sorted (now newest-first by date).
- ~~Editor's Selections' Note-type fallback title~~ — hardcoded `'Philosophical Fragment'` replaced with the real Note identity treatment (NOTE pill + excerpt in the handwritten voice font), matching Featured Notes.

### Open — not yet acted on
- Editor's Selections runs its backfill before Featured Essays/Notes and is type-agnostic, so with a small corpus it can consume all available Notes before Featured Notes gets a turn (observed live: with only 2 Notes published, both landed in Editor's Selections, leaving Featured Notes empty). Self-resolves as the corpus grows; not fixed now.
- A fully width-measured, single-line word-safe ellipsis (`ResizeObserver`-based) for narrow table-row cells (Desk's list, Index's table) — the interim fix (`truncateAtWord()` with a fixed word budget) is solid but not pixel-exact for every viewport width. Izzat flagged this as the rigorous long-term fix but not the immediate priority.

### Bugs found and fixed after the first log entry above
- **Content was missing from both URL route-sync effects in `App.tsx`** — added to `ActiveTabType`/nav/render switch when built, but never given a `/content` case in either the state→URL or URL→state effect. Reaching Content via `handleTabClick('content')` from a personal-site context (Folio/Bio/Desk, where `selectedAuthorId = currentUser.id`) left `selectedAuthorId` uncorrected — nothing resets it for Content the way the URL round-trip does for Directory/Index — while silently rewriting the URL to `/`. Navbar's own render check (`selectedAuthorId === ''` decides which link set shows) then kept showing the personal-site nav (Folio/Biography/Desk) while the main content area correctly showed Content. Added the matching case to both effects, mirroring the existing Directory/Index pattern.
- **Every `slice(0, N) + '...'` character-based truncation site cut mid-word** (worst case, a single trailing letter before the ellipsis) — reported live by Izzat. Added `truncateAtWord()` to `utils.tsx` and swept all 17 sites built/touched this session across 10 files. Card titles (e.g. Content's Essay title) now get `line-clamp-2` instead of single-line `truncate`; dense table-row titles (Desk's list) stay single-line but cut at a word boundary via `truncateAtWord()` first, with a `title` tooltip carrying the full text.
- **Nav grouping read Content as one flat set with Directory/Index** — contradicted the established mental model (Frontpage+Content is the "portal" pair; Directory+Index is the separate catalogue pair). Added a visual divider in the top bar and account dropdown, and an explicit "Frontpage" entry in the dropdown (previously only reachable via the brand wordmark).
- **9 more raw-content preview leaks found in a full-codebase sweep** (same bug class as the Folio/Frontpage/Content leaks already fixed): `EditorialNotesView.tsx`, `NoticesView.tsx`, three more spots in `FrontpageView.tsx` (news ticker, notice board, Editorial Note aside), `LandingView.tsx`'s guest splash, four spots in the admin-only `ReferenceLibrary.tsx`, `FrontpageCurationTab.tsx`'s curation picker, and `EditorialIndex.tsx`'s Note first-sentence extraction. All now route through `flattenBlocksForPreview()`.
