...
| UI System spec | 🚧 Partial — a full audit-driven "Adjung Design System v1.0" was written 2026-07-17 (color/type/spacing/elevation/motion tokens + canonical Button/Input/Card/Badge/Modal/Table/Tooltip/Navigation specs, each citing real file:line evidence from a complete component-tree audit). Currently lives as a Claude Artifact, not yet a numbered spec doc in this folder — ask Izzat before formalizing it as one, since the rollout itself (applying it to real components) hasn't started beyond the one concrete bug fix below. | Beta |
...
| Capability | Status | Required |
|---|---|---|
| Design language (serif/maroon scholarly theme) | ✅ Built (`index.css`, custom — not Material/iOS). A bespoke design system blending iOS/Fluent/Material influences was KIV as of 2026-07-16; **actively started 2026-07-17** — see the "UI System spec" row in Layer 0 above for the v1.0 audit-driven draft. One concrete bug already fixed from that audit: **169 occurrences across 26 files used Tailwind shade numbers that don't exist in the default palette** (`stone-850`, `amber-55`, `red-650`, 20 others) — these compile to no CSS at all under Tailwind's JIT, so the affected text/borders were silently falling back to inherited/transparent color. All remapped to the nearest valid shade; `tsc --noEmit` clean. | MVP |
