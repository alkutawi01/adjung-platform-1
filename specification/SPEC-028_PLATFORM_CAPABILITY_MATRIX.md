...
- **Desk's draft/published list layout**: currently large full-width boxes with a lot of empty space per row. Research (data-table-vs-cards heuristic: structured metadata like type/title/date reads as "rows," not "items") suggests a denser table-style list (WordPress/Ghost/Notion pattern) would scale better once the corpus grows past a handful of entries. Flagged for Izzat's decision, not changed.
- Editor's Selections' Note-type fallback title is a hardcoded `'Philosophical Fragment'` string rather than a real content preview (unlike Featured Notes, which does show a real excerpt) — inconsistent, low priority.
