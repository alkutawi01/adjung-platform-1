...
content = content.replace(
  /const \[featuredEntryId, setFeaturedEntryId\] = useState<string>\([\s\S]*?\);/,
...
const uiInsertRegex = /\{\/\* Highlight Article \*\/\}/;
...
content = content.replace(uiInsertRegex, uiInsert);

fs.writeFileSync(appPath, content, 'utf8');
console.log('Successfully updated Editorium UI for Editorial Selections.');
